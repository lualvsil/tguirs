...
impl Connection {
	...
	pub fn send_msg(&self, method: items::method::Method) -> Res<()> {
		...
		while start < bytes.len() {
			let n = send(self.main, &bytes[start..], MsgFlags::empty())?;
			start += n;
		}
	...
	pub fn recv_msg<T: Message+Default>(&self) -> Res<T> {
		...
		while start < msg_size {
			let n = recv(self.main, &mut msg[start..], MsgFlags::empty())?;
			start += n;
		}
	...
	pub fn recv_event(&self) -> Res<items::Event> {
		...
		let mut start = 0;
		while start < msg_size {
			let n = recv(self.event, &mut msg[start..], MsgFlags::empty())?;
			start += n;
		}
		Ok(Message::decode(msg.as_slice())?)
	}
	
	pub fn toast(&self, text: &str, long: bool) -> Res<()> {
